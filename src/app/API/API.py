...
def proposer(value):
    value = eval(value)
    node.propose_value(value, "PROPOSE")
...
def register_customer() -> tuple[Response, int, dict[str, str]]:
    ...
    try:
        ...
        event: Event = node.propose_value(dict_customer, 'REGISTER')

        while not event.exe:
            pass

        if event.can_be_executed:
            return jsonify({'descript': 'Cliente criado com sucesso'}), 200, {'Content-Type': 'application/json; '
                                                                              'charset=utf-8'}
        else:
